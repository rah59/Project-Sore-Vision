/**
  * Created by sudhakar on 2/23/17 and adopted by Raj for Project Sore Vision on 2/25/2017
  * This code creates a simple HTTP server that receives an image and passes it on
  * to Clarifai for classification as a Cold Sore or a Canker Sore
  */
...
class RootHandler extends HttpHandler {
  def handle(httpExchange: HttpExchange) {
    ...
    // Modified by Raj on 02-27-2017 to call Clarifai to classify the image
    val response = Clarifai.testImage("image.png")
